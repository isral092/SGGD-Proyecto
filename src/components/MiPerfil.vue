...
onMounted(async () => {
  try {
    ...
    if (authError || !user) {
      error.value = 'Sesión no válida o usuario no autenticado.'
      loading.value = false
      return
    }

    console.log('🔍 Buscando perfil en DB para ID:', user.id)
    ...
    if (dbError) {
      ...
      error.value = dbError.code === 'PGRST116'
        ? 'El perfil no existe en la base de datos. Contacta al administrador.'
        : `Error de acceso: ${dbError.message}`
      loading.value = false
      return
    }
  ...
  <div class="w-full h-full flex flex-col items-center justify-center px-4 py-8">
    ...
    <div v-else-if="perfil" class="w-full lg:max-w-5xl xl:max-w-6xl">
      <div class="bg-white rounded-2xl shadow-2xl p-8 lg:p-12 border-t-4 border-blue-500 flex flex-col lg:flex-row gap-8 lg:gap-16 items-center lg:items-start">
        ...
        <!-- Avatar Section (Left on Desktop) -->
